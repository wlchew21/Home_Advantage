...
out.hca1 <- 
  out.hca %>% 
  mutate(lab1 = case_when(HCA %in% c("Pts", "2P", "3P", "FT") ~ "Points",
                          HCA %in% c("FTA", "2PA", "3PA")     ~ "Attempts",
                          HCA == "Asst"                       ~ "Assists",
                          HCA == "STL" ~ "Steals",
                          HCA == "BLK" ~ "Blocks",
                          HCA == "TOV" ~ "Turnovers",
                          HCA == "PF"  ~ "Personal Fouls",
                          HCA %in% c("DRB", "ORB", "TRB") ~ "Rebounds",
                          HCA %in% c("FG_pct", "2pt_pct", "FT_pct", "3pt_pct") ~ "Shooting Percentage"))
...
hca.plot <- function(hca.param){
  dat.plot <- out.hca1 %>% filter(HCA %in% hca.param)
  mi.szn <- min(dat.plot$season)
  ma.szn <- max(dat.plot$season)
  hca.lab <- unique(dat.plot$lab1)
  
  
  ggplot(dat.plot, aes(x = season, y = Estimate)) + 
    geom_line() +
    geom_point() + 
    geom_errorbar(aes(x = season, ymin = `2.5 %`, ymax = `97.5 %`), width = 0.1) + 
    geom_smooth(fill = "lightgray") + 
    theme_minimal() + 
    ggtitle(glue::glue("HCA in {unique(hca.param)} from {mi.szn} to {max(ma.szn)}")) + 
    xlab("Season") + ylab(hca.lab)
}
